...
def train_model(X, Y, model, verbose=True):
    ...
    print(pred_test, type(pred_test), pred_test.dtype, pred_test.shape)
...
def eval_whitebox_classifier(R, g, EX, StdX, NormV, x0, label_x0, bb_classifier, wb_name,
    ...
    # p_score gibt an wie stark die das Feature die Vorhersage des Modells beeinflusst
    # positiver p_score --> postive Vorhersage 
    ...
    # X1 liget auf der Entscheidungsgrenze des Ridge Modells und am nächste zu, x0
    sx1 = sx0 + N_sx0_w
    x1 = (sx1 * StdX) + EX
...
class LEAF:
    def __init__(self, bb_classifier, X, class_names, explanation_samples=5000):
        ...
        ''' 
        num_reps: Anzahld der Wiederholungen
        neighbothood_sampels: Anzahl der Nachbarschaftsstichproben
        verbode: Boolean Wert, ob detallierte Ausgabe angezeiget werden soll
        figure_dir: Verzeichnis zum Speichern von Abbildungen
    ...
    def explain_instance(self, instance, num_reps=5, num_features=4, 
        ...
        # self.Ex enthält den Durchschnittswert aller Elemente in X und wird konvertiert in np.array
        npEX = np.array(self.EX)
        cls_proba = self.bb_classifier.predict_proba

        # instance to be explained
        x0 = copy.deepcopy(instance)
        
        print("x0 shape", np.array(x0).shape)

        mockobj = mock.Mock()

        # Neighborhood random samples
        cov_matrix = np.cov(((self.X - npEX) / self.StdX).T) if use_cov_matrix else 1.0
        ...
        # prob_x0_F ist die wahrscheinlichkeit für den falschen Output
        # prob_x0_T ist die Wahrscheinlichkeit für den richtigen Output
        ...
        # Explain the same instance x0 multiple times  --> wichtig for Retairation 
        for rnum in range(num_reps):
            ...
            # Explain x0 using SHAP, wobei die anzahl der features auf 10 begrenzt ist
            # shap values zeigen wie stark jedes Feature zur Vorhersage des Modells für x0
            #shap_phi = self.SHAPEXPL.shap_values(x0, l1_reg="num_features(10)")
            shap_phi = self.SHAPEXPL.shap_values(x0)
            print(np.array(self.SHAPEXPL.shap_values).shape)
            

            # expected value gibt an welchen Wert der Modellvorhersage im Durchschnitt haben sollte.
            ...
            for k in range(len(shap_phi)):
                # die unwichtigesten Features werden auf 0 gesetzt
            ...
            # hier wird überprüft wie gut das white box Modell das Lime bzw Shap Modell nachahmen kann.
            ...
            #  lokale Diskrepanz wird berechet und gibt an wie stark sich die Vorhersagen des Lime/ Shap vom Blackbox Modell unetrscheiden
            ...
            # coef sind die Gewichte die jedem Merkmal zugeordnet sind
            ...
            # binäre Maske um anzuzeugen welche Merkmale wichtig sind 
            # self.F -> Anzahl der Merkmale
            R.lime_bin_expl, R.shap_bin_expl = np.zeros(self.F), np.zeros(self.F)
            # np.array(R.mcf_lime) enthält die Indizes der wichtigsten Merkmale. 
            # hier weden die wichtigsten Merkmal mit 1 markiert und alle irrelevante Merkmale auf 0 gesetzt
        ...
        if prescription:
            ...
            # Show SHAP explanation
            shap_phi = self.SHAPEXPL(x0, outputs=shap.Explanation.argsort.flip[:5])
